    ...
    INxpWiredSeHalCallback.Stub mWiredSeCallback = new INxpWiredSeHalCallback.Stub() {
     ...
     private
      ...
      @Override public int openWiredSe() {
        ...
        if (NfcService.getInstance().isWiredOpen) {
            Log.i(TAG, "SE is Busy. returning..");
            return NfcService.getInstance().EE_ERROR_ALREADY_OPEN;
        }
        sNfcWiredSeHandle = NfcService.getInstance().doOpenSecureElementConnection();
        if (sNfcWiredSeHandle <= 0) {
          Log.e(TAG, "WiredSe: open secure element failed.");
          sNfcWiredSeHandle = 0;
        } else {
          NfcService.getInstance().isWiredOpen = true;
      ...
      @Override public ArrayList<Byte> getAtr(int wiredSeHandle)
        ...
        Log.d(TAG, "WiredSe: getAtr:SN100x");
